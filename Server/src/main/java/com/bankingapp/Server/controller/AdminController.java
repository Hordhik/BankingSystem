...
public class AdminController {
        ...
        public ResponseEntity<List<Map<String, Object>>> getDashboardStats() {
                ...
                BigDecimal totalRevenue = transactionRepository.findAll().stream()
                                .map(t -> {
                                        BigDecimal amount = "DEPOSIT".equals(t.getType()) ? t.getAmount()
                                                        : BigDecimal.ZERO;
                ...
                BigDecimal avgTransactionValue = activeTransactions > 0
                                ? totalRevenue.divide(BigDecimal.valueOf(activeTransactions),
                                                java.math.RoundingMode.HALF_UP)
                                : BigDecimal.ZERO;

                List<Map<String, Object>> stats = List.of(
                                Map.of("label", "Total Users", "value", String.valueOf(totalUsers), "color", "#667eea",
                                                "change",
                                                "+12%"),
                                Map.of("label", "Active Transactions", "value", String.valueOf(activeTransactions),
                                                "color", "#764ba2",
                                                "change", "+5%"),
                                Map.of("label", "Total Revenue", "value", "₹" + totalRevenue.toString(), "color",
                                                "#f093fb", "change",
                                                "+18%"),
                                Map.of("label", "Avg Transaction Value", "value", "₹" + avgTransactionValue.toString(),
                                                "color",
                                                "#4facfe", "change", "+3%"));
        ...
        public ResponseEntity<List<Map<String, Object>>> getMonthlyReports() {
                ...
                Map<String, Long> usersByMonth = allUsers.stream()
                                .collect(Collectors.groupingBy(
                                                u -> u.getCreatedAt().getMonth().getDisplayName(
                ...
                String currentMonth = LocalDate.now().getMonth().getDisplayName(java.time.format.TextStyle.FULL,
                                java.util.Locale.ENGLISH);
        ...
        public ResponseEntity<Map<String, Object>> getAnalytics() {
                ...
                int peakHour = hourCounts.entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey)
                                .orElse(12);
                String peakHourStr = (peakHour > 12 ? peakHour - 12 : peakHour) + ":00 "
                                + (peakHour >= 12 ? "PM" : "AM");

                // Busiest Day
                Map<String, Long> dayCounts = allTransactions.stream()
                                .collect(Collectors.groupingBy(t -> t.getCreatedAt().getDayOfWeek().name(),
                                                Collectors.counting()));
                String busiestDay = dayCounts.entrySet().stream().max(Map.Entry.comparingByValue())
                ...
                BigDecimal avgTxnSize = allTransactions.isEmpty() ? BigDecimal.ZERO
                                : totalAmount.divide(BigDecimal.valueOf(allTransactions.size()),
                                                java.math.RoundingMode.HALF_UP);
                ...
                response.put("topMetrics", List.of(
                                ...
                                Map.of("label", "Success Rate", "value", successRate, "subtext",
                                                "Transaction success")));
                ...
                BigDecimal hourlyAvg = totalTxns > 0
                                ...
                                : BigDecimal.ZERO;
                BigDecimal dailyAvg = totalTxns > 0
                                ? totalAmount.divide(BigDecimal.valueOf(30), java.math.RoundingMode.HALF_UP)
                                : BigDecimal.ZERO;
                BigDecimal weeklyAvg = totalTxns > 0
                                ? totalAmount.divide(BigDecimal.valueOf(4), java.math.RoundingMode.HALF_UP)
                                : BigDecimal.ZERO;
        ...
        private List<AdminTransactionDTO> mapToAdminDTOs(List<Transaction> transactions) {
                return transactions.stream().map(t -> {
                        ...
                        // User requested to remove username in brackets
                        // String username = t.getAccount().getUser().getUsername();
                        ...
                        if (t.getCounterpartyAccountId() != null) {
                                Optional<Account> counterparty = accountRepository
                                                .findById(t.getCounterpartyAccountId());
                        ...
                        dto.setCreatedAt(t.getCreatedAt().toString()); // ISO format
